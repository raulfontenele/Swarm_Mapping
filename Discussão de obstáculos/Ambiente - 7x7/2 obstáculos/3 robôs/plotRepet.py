...
def graphHex(radius):
    ...
    #plt.scatter(-coord[:,1],coord[:,0], s = 1000, c = qtdArray ,marker='h', cmap="plasma")
    #plt.colorbar()
    coord = np.array([[0,0],[0.25,0.433]])

    dist = radius/math.cos(math.pi/6)
    img = plt.imread("7x7_1O_P1.png")
    fig, ax = plt.subplots()
    ...
    ax.imshow(img,extent=[-3.5, 3.5, -3.5, 3.5])
    file = open('map.txt','r')
    lines = file.readlines()
    ...
    ax.set_xlim([-0.8,0.5])
    ax.set_ylim([-0.4,0.6])
    plt.scatter(-coord[:,1],coord[:,0], s = 300, c = "#FF0000" , marker='o')
    coord = np.array([[0.18,0.51]])
    plt.scatter(-coord[:,1],coord[:,0], s = 300, c = "#00FF00" , marker='o') 
    #plt.title("Cenário com 4 obstáculos")
    plt.grid()
